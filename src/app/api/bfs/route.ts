...
export async function POST(req: Request) {
  ...
  function breadthFirstSearch(cy: Core, startNodeId: string) {
    let visitedNodes = new Set();
    let visitedEdges = new Set();
    let visitedNodesTitles: string[] = [];
    let queue = [
      { prevNodeId: "",nodeId: startNodeId, edgeId: "" },
    ];
    let frames = [];
    let stepByStepExplanation: string[] = [];

    while (queue.length > 0) {
      const { prevNodeId, nodeId, edgeId } = queue.shift()!;
        ...
        sortedNeighbors.forEach((neighbor) => {
          if (!visitedNodes.has(neighbor.id())) {
            ...
            if (!isDirected || edge.source().id() === nodeId) {
              queue.push({ prevNodeId: nodeId, nodeId: neighbor.id(), edgeId: edge.id() });
            }
          }
        });
      }
    ...
    const resultText = `### Результат выполнения алгоритма поиска в ширину (BFS)
...
**Узнать больше об алгоритме поиска в ширину можно по следующей [ссылке](https://ru.wikipedia.org/wiki/Поиск_в_ширину).**`;

    return { frames, shortResultText, resultText, stepByStepExplanation };
  }

  const { frames, shortResultText, resultText, stepByStepExplanation } = breadthFirstSearch(cy, startNodeId);
