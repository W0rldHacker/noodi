  ...
  useEffect(() => {
    const handleHotkey = (event: KeyboardEvent) => {
      if (
        ...
        clearGraph();
      } else if (!algorithmMode && event.ctrlKey && (event.key === "g" || event.key === "п")) {
        event.preventDefault();
        toggleGrid();
      } else if (!algorithmMode && event.ctrlKey && (event.key === "v" || event.key === "м")) {
        event.preventDefault();
        toggleAddNodeMode();
      } else if (!algorithmMode && event.ctrlKey && (event.key === "e" || event.key === "у")) {
        event.preventDefault();
        toggleAddEdgeMode();
        ...
        >
          <ul className="menu menu-horizontal items-center bg-base-content text-base-200 bg-opacity-50 backdrop-blur rounded-box">
            <li>
              ...
              >
                <button className="p-2 rounded-lg" onClick={toggleAddNodeMode}>
            ...
            <li>
              ...
              >
                <button className="p-2 rounded-lg" onClick={toggleAddEdgeMode}>
            ...
            <li>
              <a
                className="tooltip h-9 w-9 before:bg-transparent before:text-xs before:mb-2 after:hidden"
                data-tip="Настройка графа (Ctrl + T)"
              >
                <button>
                  <FaSliders size={20} />
                </button>
